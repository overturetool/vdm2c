...
public class GenerateClassDispatchTrans extends DepthFirstAnalysisCAdaptor
{
			...
			throws AnalysisException
	{
		...
		if (node.getTag() != null)
		{
			...
			Set<SExpIR> dm = SystemArchitectureAnalysis.distributionMap.get(node.getTag());
			...
			for(SExpIR d : dm){
				...
				STypeIR ty = ((AExplicitVarExpIR) d).getType();

				li.add(((AClassTypeIR) ty).getName());	

				if(d instanceof AExplicitVarExpIR){
					AExplicitVarExpIR a = (AExplicitVarExpIR) d;

					if(d.getType().getSourceNode().getVdmNode() instanceof AOptionalType){
						AOptionalType classNN = (AOptionalType) d.getType().getSourceNode().getVdmNode();
