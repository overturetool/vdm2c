...
DepthFirstAnalysisCAdaptor
{
	...
	public void caseAFieldExpIR(AFieldExpIR node) throws AnalysisException
	{
		...
		for(SClassDeclIR c : assist.getInfo().getClasses())
		{
			if(fieldUtil.lookupFieldClass(c,  node.getMemberName()) != null)
			{
	...
	//Need to determine whether the identifier expression refers to a field or to some other variable.  Can be done by searching through
